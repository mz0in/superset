...
def test_email_chart_report_schedule_with_cc_bcc(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            '%7D&force=false">Explore in Zain Analytics </a>' in email_mock.call_args[0][2]
        )
...
def test_email_chart_report_schedule(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            '%7D&force=false">Explore in ZReport</a>' in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
        # Assert the email inline screenshot
...
def test_email_chart_report_schedule_force_screenshot(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            '%7D&force=true">Explore in ZReport</a>' in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
        # Assert the email inline screenshot
...
def test_email_chart_report_schedule_with_csv(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            'force=false">Explore in ZReport</a>' in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
...
|  1 | c21  | c22  | c23       |"""
        ...
        assert (
            f"<http://0.0.0.0:8080/explore/?form_data=%7B%22slice_id%22:+{create_report_slack_chart_with_text.chart.id}%7D&force=false|Explore in Zain Analytics >"
            in slack_client_mock_class.return_value.chat_postMessage.call_args[1][
                "text"
            ]
        )
